...
//      }
    ...
    @Override
    public String deleteBackups(DatabaseEntity databaseEntity) throws Exception {
        StringBuilder commond = new StringBuilder();
	    //获取保存路径
        String backPath = databaseEntity.getSavePath();

        //获取路径下的所有文件
        Date currentDate = new Date();
        commond.append(" mkdir ").append(backPath + " ");   //切换文件夹
        commond.append(" rm -f !(").append("").append(") ");  //删除文件

        //执行命令
        log.info("restoreDatabase:[{}]", commond.toString());
        UserInfo userInfo = new UserInfo();
        userInfo.setIp(databaseEntity.getIp());
        userInfo.setUsername(databaseEntity.getUser());
        userInfo.setPassword(databaseEntity.getPassword());
        SessionProxy sessionProxy = SessionFactory.getInstance().getCommandOperation(userInfo).getSessionProxy();
        ShellExecReturn shellExecReturn = sessionProxy.execReturn(commond.toString(), 20 * 1000L);
        log.info("ShellExecReturn为[{}]",shellExecReturn);
        if (shellExecReturn.getStatus() == 0) {// 0 表示线程正常终止。
            return "数据库还原成功";
        }else{
